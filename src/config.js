export default {
  navbar: {
    title: "ARCMEDIA",
    topImage: "/favicon.ico",
    links: [
      { title: "WHAT WE DO", to: "#what-we-do" },
      { title: "OUR TEAM", to: "#our-team" },
      { title: "OUR CLIENTS", to: "#our-clients" },
      { title: "WHO'S TALKING", to: "#whos-talking" },
      { title: "OUR WORK", to: "#our-work" },
      { title: "TESTIMONIALS", to: "#testimonials" },
      { title: "SERVICES", to: "#why-we-different" },
      { title: "CONNECT", to: "#connect" }
    ]
  },
  work: [
    "/images/showcase/denimstar-1-min.jpg",
    "/images/showcase/denimstar-2-min.jpg",
    "/images/showcase/denimstar-3-min.jpg",
    "/images/showcase/k9-1-min.jpg",
    "/images/showcase/punjabispice-1-min.jpg",
    "/images/showcase/punjabispice-2-min.jpg",
    "/images/showcase/repsfitness-1-min.jpg",
    "/images/showcase/repsfitness-2-min.jpg",
    "/images/showcase/spacebar-1-min.jpg",
    "/images/showcase/spacebar-2-min.jpg",
    "/images/showcase/spacebar-3-min.jpg",
    "/images/showcase/spacebar-4-min.jpeg",
    "/images/showcase/stark-1-min.jpeg",
    "/images/showcase/stark-2-min.jpeg",
    "/images/showcase/stark-3-min.jpeg",
    "/images/showcase/stark-4-min.jpeg",
    "/images/showcase/velocity-1-min.jpg"
  ],
  family: [
    "/images/our-family/asa-min.png",
    "/images/our-family/denimstar-min.jpg",
    "/images/our-family/k9-min.jpeg",
    "/images/our-family/myxibulls-min.png",
    "/images/our-family/nutrikhata-min.jpg",
    "/images/our-family/punjabispice-min.png",
    "/images/our-family/repsfitness-min.png",
    "/images/our-family/spacebar-min.png",
    "/images/our-family/velocity-min.jpg"
  ],
  whosTalking: [
    "/images/work-with/fllorence-min.png",
    "/images/work-with/image1-min.jpeg",
    "/images/work-with/indra.gif",
    "/images/work-with/patiala-min.png",
    "/images/work-with/pune-hip-hop-min.jpeg",
    "/images/work-with/saibalaji-min.jpeg",
    "/images/work-with/StarCafe-min.png",
    "/images/work-with/tedx-min.png",
    "/images/work-with/woodpecker-min.png"
  ],
  socials: [
    { name: "Facebook", icon: "logo-facebook", link: "https://www.facebook.com/ARC-MEDIA-100112811472596/", text: "" },
    { name: "Phone", icon: "call-outline", link: "tel:+919764865849", text: "" },
    {
      name: "Email",
      icon: "mail-outline",
      link: "mailto:arcmediaventures@gmail.com",
      text: ""
    },
    { name: "Instagram", icon: "logo-instagram", link: "https://instagram.com/arcmediaventures", text: "" }
  ],
  members: [
    {
      name: "Vivian Anthony",
      img: "/images/team/VivianAnthony-min.jpg",
      position: "CEO and FOUNDER",
      about:
        "A full-time hardworking extrovert with the expertise of 5 years in the marketing, advertising, and communication industry; Vivian knows exactly what your business needs and requires at each and every given time of the year. From the planning of an event, brainstorming behind each and every aspect of the marketing technique that should be used accurately; behind your business, Vivian knows and covers it all."
    },
    {
      name: "Yash Aryan",
      img: "/images/team/Bunny-min.jpg",
      position: "Director of photography / D.O.P",
      about:
        "A master of camera handling and a production genius, Yash covers it all. With an experience of 5 years in the field of commercial production, conceptualized shoots, wedding, and other product shoots; Yash makes a mark in the core team of Arc Media."
    },
    {
      name: "Sagar Surkute",
      img: "/images/team/sagar.jpg",
      position: "post production manager",
      about:
        "The head chef of Arc Media’s kitchen, who takes care of the post-production behind every shoot that takes place. Furnishing all the work with his hands dipped in the juice of vision and creativity."
    },
    {
      name: "Shivani Tiwari",
      img: "/images/team/ShivaniTiwari-min.jpg",
      position: "public relations manager",
      about:
        "Shivani is the Public Relation manager who is hardworking and is responsible for managing all. From maintaining cordial relations with clients to building a strong, excellent public image of brands,Shivani does it all."
    },
    {
      name: "Rishab Solanki",
      img: "/images/team/rishab.jpg",
      position: "Assistant Director of Photography",
      about:
        "With 3 years in the Industry, Rishab has wide experience in fashion photography, traditional photography and commercial product shoots. He has the versatility to play roles on the shoot. He is hardworking and determined to learn."
    },
    
    {
      name: "Tushar Kudale",
      img: "/images/team/tushar.jpg",
      position: "Graphic designer",
      about:
        "Print designs to hoarding designs to logo designs, Tushar covers it all with 3 years of experience in graphic designing for Brands. All the design requirements for Branding is fulfilled by him. He is consistent, knows to work under pressure. He puts creativity to work and  adds vision to designs for clients."
    },
    {
      name: "Surabhi Panicker",
      img: "/images/team/surabhi.jpg",
      position: "Public Relations Executive",
      about:
        "Public relations executive Surabhi is perfect in what she does. All the content that is generated for the clients is inspected under her. She is excellent in handling social media and takes care in maintaining relations with the client."
    }
  ],
  testimonials: [
    {
      image: "/images/our-family/repsfitness-min.png",
      company: "Reps Fitness",
      review:
        "I am very happy with the work that they provide. They have a great team with enthusiastic people always running about, getting work done. The package that they provide is quite different, which is why I picked them in the first place."
    },
    {
      image: "/images/our-family/punjabispice-min.png",
      company: "Punjabi Spice",
      review:
        "They have an amazing team. The services that they provide, right from the creatives to the cinematic videos are just fabulous. I am completely satisfied with the work they give. My social media account looks perfect."
    },
    {
      image: "/images/our-family/spacebar-min.png",
      company: "Space Bar",
      review:
        "I love working with energetic, hardworking people, and the team of Arc media is definitely like that. The representation of my business is done really well. They always come up with great ideas, which I love. I m happy with their work. Plus their service to the clients for all the doubts is very quick and prompt which makes it easier to work along."
    },
    {
      image: "/images/our-family/k9-min.jpeg",
      company: "K9",
      review:
        "I have been running a seafood restaurant in Pimple Saudagar for a while now. Arc media not just promotes the USP of our business but makes everything equally valuable. I could have never imagined the interiors of the restaurant being portrayed so beautifully on social media platforms, but Arc media made it possible. They have been a support system for us to do better . Arc Media make us reach to people's heart. We look forward to this."
    },
    {
      image: "/images/work-with/StarCafe-min.png",
      company: "Stark cafe",
      review:
        "Arc Media has helped us make our superhero-themed cafe an exclusive one in town. Every superhero needs a break, a concept by the team helped us create our presence not only on social media platforms but also in people's heart. It has been a beautiful journey with Arc Media from the very start."
    },
    {
      image: "/images/our-family/denimstar-min.jpg",
      company: "Denim Star",
      review:
        "Arc Media has always been able to meet our needs. They are truly excellent partners. They send us things before we even know that we need them. They've cared about how well we do since day 1. I've experienced nothing but complete professionalism from Arc Media."
    },
    {
      image: "/images/our-family/velocity-min.jpg",
      company: "Velocity",
      review:
        "Arc Media has really amazed us every step of the way. They provide a tremendous amount of quality content. They're flexible with scheduling. The team provides a clear road map for what they want to accomplish and how they plan to do it. We appreciate their accountability. We're so grateful to have a marketing firm working with us, not for us."
    }
    // {
    //   name: "",
    //   image: "/images/our-family/",
    //   company: "",
    //   review: ""
    // }
  ],
  services: [
    {
      title: "1. Digital marketing",
      para:
        "We keep up with the latest marketing insights, google data, and insights. Digital marketing is the component of marketing that utilizes the internet and online-based digital technologies such as desktop computers, mobile phones, and other digital media and platforms to promote products and services."
    },
    {
      title: "2. Social media marketing",
      para: "Wherein we cover all social media platforms including Instagram, Facebook, Youtube, LinkedIn."
    },
    {
      title: "3. Outdoor marketing",
      para:
        "We also provide outdoor media and outdoor marketing from designing billboards, hoarding to getting your word, and your designs all over."
    },
    {
      title: "4. Celebrity management",
      para:
        "We are the publicist and the contract negotiator for our clients under celebrity management. We take care of their p.r services, social media handling to providing all the press, and the media coverage any celebrity needs to be present in the market."
    },
    {
      title: "5. Model management",
      para: "We connect new faces and models with different scouts that work in the fashion industry."
    },
    {
      title: "6. Customer relationship management",
      para: "Arc Media has an approach to managing a company’s interaction with current and potential customers."
    },
    {
      title: "7. Production services",
      para:
        "We provide various commercial graphic designs, commercial photography, commercial videography, hosting all types of different events, and later making a cinematic and a motion graphic video of the entire event setup buy Arc Media"
    },
    {
      title: "8. Push notification services",
      para:
        "A service which is related to SMS marketing, WhatsApp marketing, Messenger marketing to spread the reach and goodwill of our client to a vast number of audience present in the market with the available amount of potential leads that Arc Media holds as a firm in the versatile field of industries that we engage in."
    },
    {
      title: "9. Public relations",
      para: "Arc Media has the best of public relations of the P.R executives and managers with experience in strategic P.R consulting."
    },
    {
      title: "10. Wedding and event management",
      para:
        "Arc Media is extremely organized and strategic when it comes to event planning. May it be a wedding or any other corporate or social event, Arc Media covers it all."
    },
    {
      title: "11. Audio and visual marketing",
      para:
        "We provide audio and visual marketing expected to deliver sophisticated and integrated experiences to engage the consumer and create demand."
    },
    {
      title: "12. Corporate merchandise",
      para:
        "Corporate merchandise is products branded with a logo or slogan that ultimately makes a brand reach more people and increases interaction amongst the crowd."
    },
    {
      title: "13. Outdoor catering",
      para: "Outdoor catering involves a caterer or caterers engaged in providing services in connection with catering at different places."
    },
    {
      title: "14. Promotions",
      para:
        "Arc Media has an excellent team of promoters who are engaged in different areas of promotional activities in order for the services and the products of our clients to have a wide reach in the audience."
    },
    {
      title: "15. Corporate social responsibility activities",
      para:
        "We deal with different businesses CSR activities in the most efficient and in a brilliant way to create a strong mark and impression on the people of the society as well as activities that fit in the guidelines of the government set for different large scale businesses."
    },
    {
      title: "16. Inbound marketing",
      para: "It is a technique that is used by Arc Media for drawing customers to products and services via social media marketing."
    },
    {
      title: "17. Content writing",
      para:
        "Arc Media also engages in content writing where different type of content is generated for different businesses for the purpose of serving different media platforms."
    },
    {
      title: "18. Branding and strategic branding activities",
      para:
        "Our branding and brand strategies make us simple and the only one in the market. We cater to the right audience and create our space in customer’s psychology and help our clients grow."
    },
    {
      title: "19. Corporate Training",
      para:
        "Under Corporate training, traning of Body Language,Personality development, Communication skills, Sales enhancement traning, Voice modulation training, is done. Training of understanding Human behaviouristics patterns which help an employee of subordinate or managerial level to grow at each and every point in their work environment. Also, Arc Media helps employees of different organisation to understand how to use their individuals interpersonal skills in order to interact and behave with the employees working in a coordinated environment."
    }
  ]
};

/*

images go in ./public/images/ folder
image links reletive to ./public/
icons for socials from here https://ionicons.com/

Some common icon names:
logo-twitter
logo-facebook
logo-whatsapp
logo-youtube
logo-snapchat
logo-instagram
logo-linkedin
logo-google-playstore
mail-outline
call-outline

*/

// build command: npm run build
// publish directory: build
